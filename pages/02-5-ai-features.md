...
![레퍼런스 문답 편집](../assets/ui/dialog-reference-editor.png)
...
![감정별 추가 프롬프트 — 추가 프롬프트 탭](../assets/ui/dialog-emotion-prompt-extra.png)

**추가 프롬프트** 탭에서 감정·강도(low/mid/high)별 문장을 넣습니다.

![감정 분석 프롬프트 탭](../assets/ui/dialog-emotion-prompt-analyzer.png)

**감정 분석 프롬프트** 탭에서는 유저 발화의 감정 변화량(delta)을 판별하는 LLM용 `emotion_prompt.txt`를 편집합니다.

표정 연결은 [라투디](https://wikidocs.net/372532)의 **감정→표정 매핑**과 짝을 이룹니다.

## 장기 메모리·요약

**장기 메모리 (대화 내용 기억)** — 과거 대화에서 중요한 내용을 골라 기억해, 이후 대화에 반영합니다. **AI 기능** 탭에서 켜고, **AI 모델** 탭에 임베딩용 API 키(chatgpt/gemini)가 있어야 합니다. 캐릭터 폴더의 `db/`에 자동 저장됩니다.

**대화 후 세션 요약 자동 갱신** — 대화가 끝날 때 <span class="tex2jax_ignore">$SUMMARY</span> 매크로를 짧게 갱신해, 다음 턴에서 「지금까지 무슨 얘기했는지」 맥락을 유지합니다.
...
**풀 프롬프트 출력** — 체크하면 콘솔 로그에 LLM에 보낸 전체 system/user 프롬프트를 출력합니다. 말투·변수 치환이 이상할 때 확인용입니다.
