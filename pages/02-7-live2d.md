...
WebSocket(`localhost:8765`)으로 모델·표정·의상이 Unity에 반영됩니다. [[01. 시작하기|Unity 클라이언트]] 연결 상태를 확인하세요.
...
LLM이 `<happy>` 같은 감정 태그를 내면, [[02-5. AI 기능|감정별 프롬프트]]와 아래 **감정→표정 매핑**을 거쳐 Live2D 표정이 연결됩니다.
...
캐릭터 목소리 wav·대사는 [[02-3. 오디오·음성|오디오·음성]] 탭 **보이스 설정**에서 지정합니다. 라투디 탭의 **재생 설정**으로 볼륨·립싱크 감도를 Unity에 맞출 수 있으며, 연결된 상태면 **실행 중 즉시** 반영됩니다.
