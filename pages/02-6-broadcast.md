...
채팅 반응용 **시나리오**는 [[02-2. 캐릭터|캐릭터]] 탭에서 고릅니다.
...
- **댓글 오버레이 노출 테스트** — Unity 연결 상태에서 샘플 채팅 표시
...
고급 옵션 `react_to_comments_immediately` — 댓글 수신 즉시 큐 투입 (`config.yaml`).
...
AI가 댓글/후원에 **응답하기 시작할 때** Unity로 overlay 이벤트가 전송됩니다. OBS 창 캡처 시 자막·오버레이가 함께 녹화됩니다.
