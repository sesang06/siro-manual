# 04. 문제 해결

## 캐릭터·자막·소리가 없음

**증상:** 캐릭터 창이 안 뜨거나, 자막·TTS가 나오지 않음

**확인:**

1. `main_pyqt.exe`를 실행했는지 (캐릭터 창이 함께 떠야 함)
2. 우측 **프로그램 시작**을 눌렀는지
3. 프로그램을 한 번 종료했다가 다시 실행해 보기

## STT가 동작하지 않음

**증상:** 말해도 AI가 반응하지 않음

**확인:**

1. **마이크 뮤트**가 꺼져 있는지
2. **오디오·음성** 탭에서 올바른 마이크 선택
3. **STT 테스트** (프로그램 시작 전)로 인식 확인
4. OpenAI Realtime: API 키·인터넷 연결
5. Whisper: CUDA/CPU 부하 — 더 작은 모델 시도

**OpenAI Realtime:** delta는 오는데 최종 텍스트가 없을 때 — 말을 멈춘 뒤 1~2초 대기 (침묵 후 commit). **풀 프롬프트 출력**을 켜고 로그 확인.

## AI 발화 중 마이크

**AI 발화 중 마이크 음소거**가 켜져 있으면 TTS 동안 STT가 꺼집니다. 끄거나 TTS 종료 후 다시 시도하세요.

## TTS / GPT-SoVITS

**증상:** AI는 응답하지만 소리가 없음

**확인:**

1. **프로그램 시작**을 눌렀는지
2. `tts 모듈` 사용 시 GPT-SoVITS 서버 기동 여부 (로그 확인)
3. **라투디** / 보이스 패널 볼륨
4. Supertone/MiniMax: API 키·voice_id

## 자막이 너무 빨리 사라짐

- TTS **재생 중**에는 현재 문장이 유지됩니다
- **재생 종료 후** 약 5초간 유지된 뒤 사라집니다
- 한 번에 **한 문장만** 표시됩니다

## 자막이 두 문장 겹침

최신 배포 zip을 사용하면 문장마다 **교체** 표시됩니다. 구버전이면 Hugging Face에서 새 zip을 받아 보세요.

## 댓글/후원 무반응

1. **방송** 탭에서 해당 체크박스 ON
2. URL이 올바른지 (위플랩 오버레이 전체 URL)
3. AI가 말하는 중이면 큐에 대기 — TTS 끝난 뒤 처리
4. 상세 설정에서 필터·차단 키워드 확인

## API 키 / LLM 오류

- **AI 모델** 탭 API 키 확인
- **풀 프롬프트 출력** 체크 후 콘솔 로그
- 모델명 deprecated 여부 — 최신 기본 모델 사용

## 설정이 저장되지 않음

UI 값은 대부분 자동 저장됩니다. **재시작 필요** 항목(모델·STT provider·TTS 모듈)은 프로그램을 다시 시작해야 합니다.

## 장기 메모리

**증상:** 장기 메모리를 켰는데 AI가 과거 대화를 거의 기억하지 않음

**확인:**

1. **AI 기능** 탭 → **장기 메모리 (대화 내용 기억)** ON
2. **AI 모델** 탭 → 임베딩용 API 키 (chatgpt/gemini)
3. 대화가 쌓인 뒤에야 검색됩니다 — 처음 몇 턴에서는 비어 있을 수 있음
4. **풀 프롬프트 출력**을 켜고 MEMORY 매크로 치환 내용 확인

## 더 도움이 필요할 때

- [변경 이력](https://wikidocs.net/372523) — 최근 수정 사항
- 배포 버전: Hugging Face 버킷의 `version.yaml` 메타
